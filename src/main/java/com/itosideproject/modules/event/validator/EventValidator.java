package com.itosideproject.modules.event.validator;

import com.itosideproject.modules.event.Event;
import com.itosideproject.modules.event.form.EventForm;
...
public class EventValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (isNotValidEndEnrollmentDateTime(eventForm)) {
            errors.rejectValue("endEnrollmentDateTime", "wrong.datetime", "처리 접수 종료 일시를 정확히 입력하세요.");
        }

        if (isNotValidEndDateTime(eventForm)) {
            errors.rejectValue("endDateTime", "wrong.datetime", "처리 종료 일시를 정확히 입력하세요.");
        }

        if (isNotValidStartDateTime(eventForm)) {
            errors.rejectValue("startDateTime", "wrong.datetime", "처리 시작 일시를 정확히 입력하세요.");
        }
    }
